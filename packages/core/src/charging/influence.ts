...
export interface InfluenceResult {
  ...
  areas: Float64Array;
  /** Polígono usado como límite (perímetro dado o contorno automático) en coordenadas de entrada. */
  boundary: Vec2[];
}

/**
 * Área de influencia de cada taladro: celda de Voronoi de las bocas recortada al perímetro.
 * Sin perímetro se usa la envolvente convexa expandida media distancia típica entre vecinos
 * (o un rectángulo si los taladros son colineales). Taladros coincidentes comparten 0 m².
 */
export function influenceAreas(
  ...
  if (n === 0) return { areas, boundary: boundary ? [...boundary] : [] };
  ...
  if (boundary && boundary.length >= 3) {
    local = boundary.map((p) => ({ x: p.x - ox, y: p.y - oy }));
  } else {
    const nn: number[] = [];
    for (let i = 0; i < n; i++) {
      let best = Infinity;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!cell) continue; // punto duplicado
    const cellPoly = cell.slice(0, -1).map(([x, y]) => ({ x, y }));
    areas[i] = Math.abs(polygonSignedArea(clipPolygonConvex(local, cellPoly)));
  }
  return { areas, boundary: local.map((p) => ({ x: p.x + ox, y: p.y + oy })) };
}
